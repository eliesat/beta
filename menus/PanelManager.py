...
# ---------------- Plugin path / unlock marker ----------------
PLUGIN_PATH = os.path.dirname(__file__)
UNLOCK_FLAG = os.path.join(PLUGIN_PATH, "eliesat_unlocked.cfg")
...
class PanelManager(Screen):
    ...
    def __init__(self, session):
        ...
        # ---------------- Directory (original plugin behavior) ----------------
        ...
        self["actions"] = ActionMap(
            ["OkCancelActions", "ColorActions", "DirectionActions"],
            {
                ...
                "left": self.cycle_left,   # cycle original folders
                "right": self.cycle_right   # cycle original folders
            }, -1
        )

        # If already unlocked, prefill password internally
    ...
    def _refresh_fields_and_focus(self):
        ...
        if sel == "username":
            ...
            hint = "Selected: Password  (OK to edit)"
        else:
            hint = "Selected: Folder  (OK or Red to apply; Left/Right to change)"
    ...
    # ---------------- Apply / auth (original folder behavior) ----------------
    def apply_dir(self):
        if not is_unlocked():
    ...
    # ---------------- Browse / cycle (original folder behavior) ----------------
    def browse_dir(self):
        try:
