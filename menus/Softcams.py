...
class FlexibleMenu(GUIComponent):
    ...
    def buildEntry(self):
        ...
        if "addons" not in self._cached_logos:
            logoPath = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/softcams.png")
...
# ---------------- S O F T C A M S   C L A S S ----------------
INSTALLER_URL = "https://raw.githubusercontent.com/eliesat/beta/main/installer.sh"
EXTENSIONS_URL = "https://raw.githubusercontent.com/eliesat/eliesatpanel/refs/heads/main/sub/softcams"
LOCAL_EXTENSIONS = "/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/assets/data/softcams"
...
class Softcams(Screen):
       ...
       def __init__(self, session):
              ...
              try:
                     ...
                     self.skin = """<screen name="Softcams" position="center,center" size="1280,720">
              ...
              # ---------------- Load Icon ----------------
              try:
                     icon_path = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/softcams.png")
       ...
       def run_selected_script(self):
              try:
                     ...
                     if not os.path.exists(LOCAL_EXTENSIONS):
                            print("[Softcams] extensions file missing")
                            return

                     script_url = self._find_script_url(pkg_name)
                     if not script_url:
                            print("[Softcams] No script found for", selected_label)
                            return
                     ...
                     print("[Softcams] run_selected_script error:", e)
       ...
       def _find_script_url(self, pkg_name):
              try:
                     ...
                     print("[Softcams] _find_script_url error:", e)
                     return None
       ...
       def update_extensions_from_github(self):
              try:
                     ...
                     if response.status_code != 200:
                            print(f"[Softcams] Failed to fetch extensions: {response.status_code}")
                            return False
                     ...
                     if local_hash == new_hash:
                            print("[Softcams] Extensions already up-to-date")
                            return False

                     with open(LOCAL_EXTENSIONS, "wb") as f:
                            f.write(response.content)

                     print("[Softcams] Extensions file updated from GitHub")
                     ...
                     print("[Softcams] update_extensions_from_github error:", e)
                     return False
       ...
       def check_plugin_update(self):
              try:
                     ...
                     if req.status_code != 200:
                            print("[Softcams] Failed to fetch installer for update check")
                            return
                     ...
                     print("[Softcams] check_plugin_update error:", e)
       ...
       def install_plugin_update(self, answer=False):
              ...
              self.session.open(
                     ...
                     finishedCallback=lambda r: print("[Softcams] Plugin update finished:", r),
                     closeOnSuccess=True,
              )
