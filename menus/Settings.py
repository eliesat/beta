...
class Settings(Screen):
	def __init__(self, session):
		...
		self.intInfo()
		t = Timer(0.5, self.update_settings)
		t.start()
	...
	def mList(self):
		...
		self.list.append((_("Channels"), 1, _("ملفات قنوات"), a))
		self.list.append((_("Frequencies"), 2, _("ملفات ترددات"), a))
		self.list.append((_("Tuner settings"), 3, _("اعدادات التيونر"), a))
	...
	def update_settings(self):
		import requests
		url = 'https://raw.githubusercontent.com/eliesat/eliesatpanel/refs/heads/main/sub/settings'
		response = requests.get(url)
		file_Path = '/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/settings'
...
def status_path():
	if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/settings"):
		status = "/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/settings"
	return status
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Settings" in line:
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Freq" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Tuner" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
