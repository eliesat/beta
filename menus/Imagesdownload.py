	...
	def select_item(self, item):
		if item:
			if item is 1:
				...
				self.session.open(Opendroid)
			elif item is 5:
				self.session.open(Openhdf)
			elif item is 6:
...
def status_path():
	if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/imagesd"):
		status = "/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/imagesd"
	return status
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Dro" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Hdf" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
