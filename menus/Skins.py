...
class FlexibleMenu(GUIComponent):
    ...
    def buildEntry(self):
        ...
        if "addons" not in self._cached_logos:
            logoPath = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/skins.png")
...
EXTENSIONS_URL = "https://raw.githubusercontent.com/eliesat/eliesatpanel/refs/heads/main/sub/skins"
LOCAL_EXTENSIONS = "/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/assets/data/skins"
...
class Skins(Screen):
    ...
    def __init__(self, session):
        ...
        try:
            ...
            self.skin = """<screen name="Skins" position="center,center" size="1280,720">
        ...
        # ---------------- Load Icon ----------------
        try:
            icon_path = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/skins.png")
    ...
    def load_main_menu(self):
        ...
        self.main_categories = [
            ("SkinsParts", "SkinsParts", "Sparts"),
            ("Oebased", "Oebased", "Soebased"),
            ("Egami", "Egami", "Segami"),
            ("Openatv", "Openatv", "Satv"),
            ("Openblackhole", "Openblackhole", "Sobh"),
            ("Openpli-8.3", "Openpli-8.3", "Spli2"),
            ("Openpli-based", "Openpli-9.1 Areadeltasat Nonsolosat Satlodge", "Spli3"),
            ("Openspa", "Openspa", "Sspa"),
            ("Openvix", "Openvix", "Svix"),
            ("Pure2", "pure2", "Spure"),  # ✅ capital P and correct status
        ]
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith("Package:"):
                    ...
                    statuses = [s.lower() for s in st.replace(",", " ").split()]
                    if status.lower() in statuses:  # <-- Fixed match for Pure2 / Spure
    ...
    def run_selected_script(self):
        try:
            ...
            if not os.path.exists(LOCAL_EXTENSIONS):
                print("[Skins] extensions file missing")
                return

            script_url = self._find_script_url(pkg_name)
            if not script_url:
                print("[Skins] No script found for", selected_label)
                return
            ...
            print("[Skins] run_selected_script error:", e)
    ...
    def _find_script_url(self, pkg_name):
        try:
            ...
            print("[Skins] _find_script_url error:", e)
            return None
    ...
    def update_extensions_from_github(self):
        try:
            ...
            if response.status_code != 200:
                print(f"[Skins] Failed to fetch extensions: {response.status_code}")
                return False
            ...
            if local_hash == new_hash:
                print("[Skins] Extensions already up-to-date")
                return False

            with open(LOCAL_EXTENSIONS, "wb") as f:
                f.write(response.content)

            print("[Skins] Extensions file updated from GitHub")
            ...
            print("[Skins] update_extensions_from_github error:", e)
            return False
    ...
    def check_plugin_update(self):
        try:
            ...
            if req.status_code != 200:
                print("[Skins] Failed to fetch installer for update check")
                return
            ...
            print("[Skins] check_plugin_update error:", e)
    ...
    def install_plugin_update(self, answer=False):
        ...
        self.session.open(
            ...
            finishedCallback=lambda r: print("[Skins] Plugin update finished:", r),
            closeOnSuccess=True,
        )
