...
class FlexibleMenu(GUIComponent):
    ...
    def buildEntry(self):
        ...
        if "addons" not in self._cached_logos:
            logoPath = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/images-backup.png")
...
class Imagesbackup(Screen):
       ...
       def __init__(self, session):
              ...
              try:
                     ...
                     self.skin = """<screen name="Imagesbackup" position="center,center" size="1280,720">
              ...
              # ---------------- Load Icon ----------------
              try:
                     icon_path = resolveFilename(SCOPE_PLUGINS, "Extensions/ElieSatPanel/assets/icons/backup.png")
       ...
       def load_main_menu(self):
              ...
              self.main_categories = [
                     ("Egami", "Egami plugins", "Ega"),
                     ("Openatv", "Openatv images", "Atv"),
       ...
       def run_selected_script(self):
              try:
                     ...
                     if not os.path.exists(LOCAL_EXTENSIONS):
                            print("[Imagesbackup] extensions file missing")
                            return

                     script_url = self._find_script_url(pkg_name)
                     if not script_url:
                            print("[Imagesbackup] No script found for", selected_label)
                            return
                     ...
                     print("[Imagesbackup] run_selected_script error:", e)
       ...
       def _find_script_url(self, pkg_name):
              try:
                     ...
                     print("[Imagesbackup] _find_script_url error:", e)
                     return None
       ...
       def _safe_open(self, screen, name):
              try:
                     ...
                     print(f"[Imagesbackup] {name} error:", e)
       ...
       def update_extensions_from_github(self):
              try:
                     ...
                     if response.status_code != 200:
                            print(f"[Imagesbackup] Failed to fetch extensions: {response.status_code}")
                            return False
                     ...
                     if local_hash == new_hash:
                            print("[Imagesbackup] Extensions already up-to-date")
                            return False

                     with open(LOCAL_EXTENSIONS, "wb") as f:
                            f.write(response.content)

                     print("[Imagesbackup] Extensions file updated from GitHub")
                     ...
                     print("[Imagesbackup] update_extensions_from_github error:", e)
                     return False
       ...
       def check_plugin_update(self):
              try:
                     ...
                     if req.status_code != 200:
                            print("[Imagesbackup] Failed to fetch installer for update check")
                            return
                     ...
                     print("[Imagesbackup] check_plugin_update error:", e)
       ...
       def install_plugin_update(self, answer=False):
              ...
              self.session.open(
                     ...
                     finishedCallback=lambda r: print("[Imagesbackup] Plugin update finished:", r),
                     closeOnSuccess=True,
              )
