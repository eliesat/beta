...
class Imagesbackup(Screen):
    ...
    def __init__(self, session):
        ...
        try:
            class_name = self.__class__.__name__.lower()  # 'imagesbackup'
            icon_path = resolveFilename(
                SCOPE_PLUGINS,
                f"Extensions/ElieSatPanel/assets/icons/{class_name}.png"
            )
            if not fileExists(icon_path):
                print(f"[Imagesbackup] Missing {icon_path}, using default.png")
                icon_path = resolveFilename(
                    SCOPE_PLUGINS,
            ...
            print("[Imagesbackup] Icon load failed:", e)

        # ---------------- FlexibleMenu ----------------
        self["menu"] = FlexibleMenu([], parent=self)
        # --- safer icon attach like Feeds/Display ---
        try:
            if hasattr(self["menu"], "setPixmap"):
                self["menu"].setPixmap(self.iconPixmap)
            else:
                self["menu"].itemPixmap = self.iconPixmap
        except Exception as e:
            print("[Imagesbackup] menu icon attach failed:", e)
        ...
        # ---------------- Background update (only extensions) ----------------
    ...
    def update_extensions_from_github(self):
        if not has_internet():
            print("[Imagesbackup] No internet: skipping extensions update")
            return False
        try:
