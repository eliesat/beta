...
class Display(Screen):
	def __init__(self, session):
		...
		self.intInfo()
		t = Timer(0.5, self.update_display)
		t.start()
	...
	def mList(self):
		...
		self.list.append((_("Bootvideos"), 1, _("فيديوهات اقلاع"), a))
		self.list.append((_("Bootlogos-Images"), 2, _("صور اقلاع خاصة بصور الانيجما"), a))
		self.list.append((_("Bootlogos-Neutral"), 3, _("صور اقلاع منوعة"), a))
		self.list.append((_("Bootlogos-Novaler"), 4, _("صور اقلاع نوفالر لكل الاجهزة"), a))
		self.list.append((_("Lcdskins"), 5, _("صور الشاشة الامامية فيو بلاس"), a))
		self.list.append((_("Radiologos"), 6, _("خلفيات لقنوات الراديو"), a))
		self.list.append((_("Spinners"), 7, _("الشعار المتحرك اعلى الشاشة يسارا"), a))
	...
	def select_item(self, item):
		if item:
			if item is 1:
				self.session.open(bvi)
			elif item is 2:
				self.session.open(displaybli)
			elif item is 3:
				self.session.open(displayble)
			elif item is 4:
				self.session.open(blno)
			elif item is 5:
				self.session.open(lcd)
			elif item is 6:
				self.session.open(rdl)
			elif item is 7:
				self.session.open(spn)
	...
	def update_display(self):
		import requests
		url = 'https://raw.githubusercontent.com/eliesat/eliesatpanel/refs/heads/main/sub/display'
		response = requests.get(url)
		file_Path = '/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/display'
...
def status_path():
	if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/display"):
		status = "/usr/lib/enigma2/python/Plugins/Extensions/ElieSatPanel/sub/display"
	return status
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Bvi" in line:
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Bli" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Ble" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
	...
	def nList(self):
		...
		for line in open(status_path()):
			if "Package:" in line:
				...
				name2 = line.replace("\n","").split(r"(\s+)")[-1]
			elif "Status:" in line and "Blno" in line:
				self.list.append((name1, name2, ipkminipng))
		self.list.sort()
		self["menu"].setList(self.list)
